...
/opt/docker_bin/docker -H unix:///opt/docker_bin/run/docker.sock run --network host --name ansible_${RANDOM} -e ANSIBLE_SSH_CONTROL_PATH=/dev/shm/ssh_ctl_%%h_%%p_%%r --rm -v `pwd`:$docker_basedir -w $docker_basedir -i$tty congcong126/ansible:2.9.18 ansible-playbook -f 5 --ssh-common-args="$ANSIBLE_SSH_COMMON_ARGS" -i $1 app_install/ovpn_changepass.yml --extra-vars "ins=$2"
